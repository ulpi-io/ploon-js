...
var require_XMLParser = __commonJS({
  "node_modules/fast-xml-parser/src/xmlparser/XMLParser.js"(exports, module) {
    ...
    var { prettify: prettify2 } = require_node2json();
...
// packages/ploon/src/encode/normalize.ts
...
function analyzeFields(objects) {
  ...
  for (const obj of objects) {
    for (const key of Object.keys(obj)) {
      ...
      const value = obj[key];
      const isEmpty2 = isJsonArray(value) && value.length === 0 || isJsonObject(value) && !isJsonArray(value) && Object.keys(value).length === 0;
      if (isEmpty2) {
...
// packages/ploon/src/encode/schema.ts
function generateSchema(data, config) {
  ...
  const schema4 = generateArraySchema(name, array, config);
  return schema4;
}
...
function analyzeArrayFields(array) {
  ...
  if (isArrayOfObjects(array)) {
    ...
    for (const key of keys) {
      ...
      const isOptional = (_a = metadata == null ? void 0 : metadata.isOptional) != null ? _a : false;
      ...
      const firstValue = (_b = array.find((obj) => key in obj)) == null ? void 0 : _b[key];
...
// packages/ploon/src/encode/path-writer.ts
var PathWriter = class _PathWriter {
  constructor(config) {
    __publicField(this, "pathSegments", []);
   ...
   */
  clone() {
    const clone = new _PathWriter(this.config);
    clone.pathSegments = [...this.pathSegments];
    return clone;
  }
};

// packages/ploon/src/shared/string-utils.ts
...
// packages/ploon/src/encode/encoder.ts
function encode(data, config) {
  const schema4 = generateSchema(data, config);
  const rootArray = findRootArray2(data);
  const records = encodeArray(rootArray, new PathWriter(config), config);
  const { recordSeparator } = config;
  const result = schema4 + recordSeparator + recordSeparator + records.join(recordSeparator);
  return result;
}
function findRootArray2(data) {
  if (isJsonArray(data)) {
    return data;
  }
  if (isJsonObject(data)) {
...
function encodeArray(array, pathWriter, config) {
  ...
  const fields = keys.map((key) => {
    var _a, _b, _c;
    const value = (_a = array.find((obj) => obj[key] !== void 0)) == null ? void 0 : _a[key];
    const isOptional = (_c = (_b = fieldMetadata.get(key)) == null ? void 0 : _b.isOptional) != null ? _c : false;
    if (isJsonArray(value)) {
...
function encodeObject(obj, fields, pathWriter, config) {
  ...
  for (let i = 0; i < complexFields.length; i++) {
    ...
    if (field.name in obj) {
      const value = obj[field.name];
      const isEmpty2 = field.type === "array" && isJsonArray(value) && value.length === 0 || // EMPTY array
      field.type === "object" && isJsonObject(value) && !isJsonArray(value) && Object.keys(value).length === 0;
      if (isEmpty2) {
        values.push("");
      }
    }
...
// packages/ploon/src/constants.ts
var PLOON_STANDARD = {
  fieldDelimiter: "|",
  pathSeparator: ":",
  arraySizeMarker: "#",
...
var DEFAULT_CONFIG = PLOON_STANDARD;

// packages/ploon/src/config.ts
...
function resolveStringifyOptions(options) {
  var _a;
  const format = (_a = options == null ? void 0 : options.format) != null ? _a : "standard";
  const baseConfig = format === "compact" ? PLOON_COMPACT : PLOON_STANDARD;
  const config = (options == null ? void 0 : options.config) ? { ...baseConfig, ...options.config } : baseConfig;
  return {
    format,
    config
  };
...
function resolveParseOptions(options) {
  ...
  return {
    ...
    config: resolveConfig(options == null ? void 0 : options.config)
  };
}
...
// packages/ploon/src/core/stringify.ts
...
// packages/ploon/src/decode/scanner.ts
function scan(ploonString, config) {
  ...
  return { schema: schema4, records };
}

// packages/ploon/src/decode/parser.ts
function parseSchema(schema4, config) {
  const {
    schemaOpen,
    schemaClose,
    fieldsOpen,
  ...
  const headerStart = schema4.indexOf(schemaOpen);
  const headerEnd = schema4.indexOf(schemaClose, headerStart);
  if (headerStart === -1 || headerEnd === -1) {
    throw new Error(`Invalid PLOON schema: ${schema4}`);
  }
  const header = schema4.slice(headerStart + 1, headerEnd);
  const [rootName, countStr] = header.split(arraySizeMarker);
  const count = countStr ? parseInt(countStr, 10) : 0;
  const fieldsStart = schema4.indexOf(fieldsOpen, headerEnd);
  if (fieldsStart === -1) {
    throw new Error(`Invalid PLOON schema: No fields found`);
  }
  const fieldsEnd = findMatchingCloseParen(schema4, fieldsStart, fieldsOpen, fieldsClose);
  if (fieldsEnd === -1) {
    throw new Error(`Invalid PLOON schema: Unmatched parentheses`);
  }
  const fieldsStr = schema4.slice(fieldsStart + 1, fieldsEnd);
...
// packages/ploon/src/decode/decoder.ts
...
function decode(ploonString, config) {
  const { schema: schema4, records } = scan(ploonString, config);
  const parsedSchema = parseSchema(schema4, config);
  const result = reconstruct(records, parsedSchema, config);
  return result;
}
function reconstruct(records, schema4, config) {
  const tree = buildTree(records, schema4, config);
  return {
    [schema4.rootName]: tree
  };
}
function buildRoutingMap(childRecords, allFields, presenceMarkers, pathSeparator) {
  ...
  for (const item of childRecords) {
    ...
    const fieldBoundary = currentBatch.length > 0 && currentIndex <= lastIndex || // Index reset
...
function buildTree(records, schema4, config) {
  var _a;
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (parsed.depth === 1 && !parsed.isObject) {
      const { obj } = createObjectFromRecord(record, schema4.fields, config);
      rootItems.push(obj);
      currentLevelParents.push({ obj, fields: schema4.fields, recordIndex: i });
    }
  }
  let currentDepth = 1;
  ...
  while (parentsAtCurrentDepth.length > 0) {
    ...
    for (const parent of parentsAtCurrentDepth) {
      ...
      const myChildrenWithIndices = childIndices.map((idx) => ({ record: records[idx], index: idx })).filter((item) => {
      ...
      for (const group of routingMap) {
        ...
        if (group.isArray) {
          ...
          for (let i = 0; i < group.records.length; i++) {
            ...
            const fields = ((_a = field.nested) == null ? void 0 : _a.fields) || [];
            if (fields.length === 0) {
  ...
  for (const item of rootItems) {
    cleanupOptionalFields(item, schema4.fields, config);
  }
  return rootItems;
}
function createObjectFromRecord(record, fields, config) {
  ...
  if (presenceMarkers.size > 0) {
    obj.__presenceMarkers__ = presenceMarkers;
  }
...
function cleanupOptionalFields(obj, fields, config) {
  ...
  for (const field of fields) {
    ...
    if (field.isArray && !field.isPrimitiveArray && Array.isArray(value)) {
      if ((_a = field.nested) == null ? void 0 : _a.fields) {
        for (const item of value) {
          if (item && typeof item === "object" && !Array.isArray(item)) {
            cleanupOptionalFields(item, field.nested.fields, config);
          }
        }
      }
      if (field.isOptional && value.length === 0 && !(presenceMarkers == null ? void 0 : presenceMarkers.has(field.name))) {
        delete obj[field.name];
      }
    } else if (field.isObject && value && typeof value === "object" && !Array.isArray(value)) {
      if ((_b = field.nested) == null ? void 0 : _b.fields) {
        cleanupOptionalFields(value, field.nested.fields, config);
      }
      if (field.isOptional && Object.keys(value).filter((k) => k !== "__presenceMarkers__").length === 0 && !(presenceMarkers == null ? void 0 : presenceMarkers.has(field.name))) {
        delete obj[field.name];
      }
    }
  }
...
// packages/ploon/src/core/parse.ts
...
// packages/ploon/src/core/minify.ts
...
// packages/ploon/src/core/prettify.ts
function prettify(ploonString) {
  const parts = ploonString.split(";");
  if (parts.length < 2) {
    return ploonString;
  }
  const [schema4, ...records] = parts;
  return `${schema4}

${records.join("\n")}`;
}

// packages/ploon/src/core/validate.ts
...
// packages/ploon/src/parsers/json.ts
...
// packages/ploon/src/parsers/xml.ts
var import_fast_xml_parser = __toESM(require_fxp(), 1);
function fromXML(input) {
  const validation = import_fast_xml_parser.XMLValidator.validate(input);
  if (validation !== true) {
...
// packages/ploon/src/parsers/yaml.ts
function fromYAML(input) {
  try {
    return browser_default.parse(input);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
function toYAML(value, pretty = false) {
  try {
    return browser_default.stringify(value, {
      indent: pretty ? 2 : 0,
      lineWidth: 0,
